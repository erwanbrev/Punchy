...
router.post('/signup', async (req, res) => {
	...
	try {
		...
		return res.status(201).send({ error: false, token });
	} catch (err) {
...
router.get('/me', authorize, async (req, res) => {
	try {
		...
		if (!user) {
			return res.status(404).send({ error: false, message: 'No user found' });
		}

		return res
			.status(200)
			.send({ error: false, user: _.pick(user, ['fName', 'lName', 'school', 'friends', 'notationCar', 'phone', 'profilePicture', 'history']) });
	} catch (err) {
		console.log(err.message);
		return res.status(500).send({ error: true, message: err.message });
	}
